...
function AppInner(){
  ...
  const handleCheckIn=(data)=>{
    ...
    if(getTodayWorkoutStatus()==="completed"){setScreen("home");setTab("home");return;}
    const loc=data?.location||"gym";
    // Ensure weekly plan exists; adjust today's pre-built plan based on check-in
    const weekPlan=getOrCreateWeeklyPlan(exerciseDB,CURRENT_PHASE,loc);
    const todayPlan=getTodayFromPlan(weekPlan);
    if(todayPlan)adjustPlanForCheckIn(todayPlan,data,exerciseDB,CURRENT_PHASE);
    // Mark today as in-progress in weekly plan
    updateDayStatus(weekPlan,getDayOfWeek(),"in_progress");
