...
return(<div style={{display:"flex",flexDirection:"column",gap:12}}>
  ...
  {!resting&&!autoAdvancing&&<div style={{display:"flex",flexDirection:"column",gap:6,position:"sticky",bottom:76,background:C.bg,padding:"12px 0",zIndex:50}}>{exMode==="none"?<div style={{display:"grid",gridTemplateColumns:"3fr 1fr",gap:8}}><Btn onClick={()=>onDone({sets:[{set_number:1,reps_done:1,load:null,rpe:null,pain:false,quality:"good"}]})} icon="✓" style={{fontFamily:"'Bebas Neue',sans-serif",letterSpacing:2}}>COMPLETE</Btn><Btn variant="dark" onClick={onSub} icon="🔄" size="sm">Swap Exercise</Btn></div>:<div style={{display:"grid",gridTemplateColumns:canUndo?"1fr 1fr 1fr":"1fr 1fr",gap:8}}>{canUndo&&<Btn variant="ghost" onClick={undoSet} size="sm" icon="↩">Undo</Btn>}<Btn onClick={handleSet} icon="✓" style={{fontFamily:"'Bebas Neue',sans-serif",letterSpacing:2}}>{cs<(ep.sets||1)?"SET DONE":"COMPLETE"}</Btn><Btn variant="dark" onClick={onSub} icon="🔄">Swap Exercise</Btn></div>}</div>}
  <div style={{height:90}}/>
</div>);}
