...
function HomeScreen({onStart,resumePrompt,onRetakeAssessment,onEditInjuries,onProfile,onViewPlan,onViewSummary,onPTSession,onPTProgress,onBaseline,onAddOn,onStartSecondary,onDevBugs,onROM,onPMROM,onPrograms,statsLoading}){const[si,setSi]=useState(null);const[debugTaps,setDebugTaps]=useState(0);const[showDebug,setShowDebug]=useState(false);const[showVO2Test,setShowVO2Test]=useState(false);const[showCardioLog,setShowCardioLog]=useState(false);const[cardioRev,setCardioRev]=useState(0);const stats=getStats();const dynamicInjuries=getInjuries().filter(i=>i.status!=="resolved");const rx=getCardioPrescription(CURRENT_PHASE,dynamicInjuries);const auth=useAuth();const userName=auth?.profile?.first_name||USER.name;const handleApexTap=()=>{const next=debugTaps+1;setDebugTaps(next);if(next>=5){setShowDebug(true);setDebugTaps(0);}setTimeout(()=>setDebugTaps(0),2000);};const easterEgg=checkEasterEgg(stats);return(<div className="stagger safe-bottom" style={{display:"flex",flexDirection:"column",gap:12}}><div style={{display:"flex",justifyContent:"space-between"}}><div><div onClick={handleApexTap} style={{fontSize:28,fontWeight:800,color:C.teal,fontFamily:"'Bebas Neue',sans-serif",letterSpacing:4,cursor:"default",userSelect:"none"}}>APEX<span style={{fontSize:9,color:C.textDim,letterSpacing:1,marginLeft:6}}>v13</span></div><div style={{fontSize:13,color:C.textMuted}}>{getGreeting(userName,stats).toUpperCase()} 👋</div>{easterEgg&&<div style={{fontSize:10,color:C.purple,marginTop:2,fontStyle:"italic"}}>{easterEgg}</div>}</div><div onClick={onProfile} style={{width:40,height:40,borderRadius:12,background:C.bgElevated,border:`1px solid ${C.border}`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:20,cursor:"pointer"}}>⚙️</div></div>
  ...
  {/* Dev-only: warn when last safety audit had failures */}
  {auth?.user?.email==="johncarrus@gmail.com"&&(()=>{try{const _last=JSON.parse(localStorage.getItem("apex_last_audit")||"null");if(!_last||_last.profilesFailed===0)return null;return(<div style={{background:"rgba(239,68,68,0.1)",border:"1px solid rgba(239,68,68,0.3)",borderRadius:8,padding:8,marginBottom:4,fontSize:11,color:"#ef4444"}}>⚠️ Last safety audit: {_last.profilesFailed} profiles failed · {_last.totalViolations} violations · open Dev tab to review</div>);}catch{return null;}})()}
  ...
  // Count
  const _items=[{done:_ptAllDone},{done:_amDone},{done:_pmRomDone},{done:!!_wkDone}];
  ...
  {/* Fitness assessment — compact one-row prompt (below workout card) */}
  {(()=>{try{if(getDismissedToday())return null;const ap=getAssessmentProgress();if(ap.completed>=ap.total&&ap.due.length===0)return null;const next=ap.nextUp;if(!next)return null;return<div onClick={onBaseline} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"10px 14px",background:C.bgCard,border:`1px solid ${C.purple}20`,borderRadius:10,marginBottom:4,cursor:"pointer"}}><div style={{display:"flex",alignItems:"center",gap:8}}><span style={{fontSize:14}}>📊</span><span style={{fontSize:12,color:C.text}}>Fitness Assessment</span><span style={{fontSize:10,color:C.textDim}}>{ap.completed}/{ap.total}</span></div><button onClick={e=>{e.stopPropagation();onBaseline?.();}} style={{padding:"4px 12px",borderRadius:6,background:C.purple+"15",border:`1px solid ${C.purple}40`,color:C.purple,fontSize:10,fontWeight:600,cursor:"pointer",fontFamily:"inherit"}}>Start</button></div>;}catch{return null;}})()}
  ...
  <CollapseSection title="Injury Protocols" icon="🩺" summary={`${getLocalProtocols().length || dynamicInjuries.length} active`}>
...
// ── ROM ROUTINE — moved to src/components/MorningROM.jsx ────────
...
function AppInner(){
  ...
  if(loading||screen==="init")return(<div style={{display:"flex",alignItems:"center",justifyContent:"center",minHeight:"100vh",background:C.bg,flexDirection:"column",gap:12}}><div style={{fontSize:48,fontWeight:800,color:C.teal,fontFamily:"'Bebas Neue',sans-serif",letterSpacing:6}}>APEX</div><div style={{width:24,height:24,border:"3px solid rgba(0,210,200,0.2)",borderTopColor:C.teal,borderRadius:"50%",animation:"spin 0.8s linear infinite"}}/><style>{`@keyframes spin{to{transform:rotate(360deg)}}`}</style></div>);
  return(<>
