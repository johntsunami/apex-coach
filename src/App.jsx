...
function HomeScreen({onStart,resumePrompt,onRetakeAssessment,onEditInjuries,onProfile,onViewPlan,onViewSummary,onPTSession,onPTProgress,onBaseline,onAddOn,onStartSecondary,onDevBugs}){const[si,setSi]=useState(null);const[debugTaps,setDebugTaps]=useState(0);const[showDebug,setShowDebug]=useState(false);const[showVO2Test,setShowVO2Test]=useState(false);const[showCardioLog,setShowCardioLog]=useState(false);const[cardioRev,setCardioRev]=useState(0);const stats=getStats();const dynamicInjuries=getInjuries().filter(i=>i.status!=="resolved");const rx=getCardioPrescription(CURRENT_PHASE,dynamicInjuries);const auth=useAuth();const userName=auth?.profile?.first_name||USER.name;const handleApexTap=()=>{const next=debugTaps+1;setDebugTaps(next);if(next>=5){setShowDebug(true);setDebugTaps(0);}setTimeout(()=>setDebugTaps(0),2000);};const easterEgg=checkEasterEgg(stats);return(<div className="stagger safe-bottom" style={{display:"flex",flexDirection:"column",gap:12}}><div style={{display:"flex",justifyContent:"space-between"}}><div><div onClick={handleApexTap} style={{fontSize:28,fontWeight:800,color:C.teal,fontFamily:"'Bebas Neue',sans-serif",letterSpacing:4,cursor:"default",userSelect:"none"}}>APEX<span style={{fontSize:9,color:C.textDim,letterSpacing:1,marginLeft:6}}>v13</span></div><div style={{fontSize:13,color:C.textMuted}}>{getGreeting(userName,stats).toUpperCase()} 👋</div>{easterEgg&&<div style={{fontSize:10,color:C.purple,marginTop:2,fontStyle:"italic"}}>{easterEgg}</div>}</div><div onClick={onProfile} style={{width:40,height:40,borderRadius:12,background:C.bgElevated,border:`1px solid ${C.border}`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:20,cursor:"pointer"}}>⚙️</div></div>
  ...
  {/* Daily task hub — compact summary */}
  {(()=>{try{const _tasks=[];const _ap=getAssessmentProgress();if(_ap.due.length>0)_tasks.push({label:`${_ap.due[0].name} (assessment due)`,icon:"📊",done:false,action:onBaseline});const _inj=getInjuries().filter(i=>i.status!=="resolved");if(_inj.length>0)_tasks.push({label:`PT exercises (${_inj.length} protocol${_inj.length>1?"s":""})`,icon:"🩺",done:false});_tasks.push({label:"Daily ROM routine (5 min)",icon:"🧘",done:false});const _todayDone=isTodayComplete();_tasks.push({label:`Today's workout${_todayDone?" — done":""}`,icon:_todayDone?"✅":"💪",done:!!_todayDone,action:onStart});const doneCount=_tasks.filter(t=>t.done).length;if(_tasks.length<=1)return null;const nextTask=_tasks.find(t=>!t.done);return<Card style={{padding:12,marginBottom:4,borderColor:C.teal+"20"}}><div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}><div style={{display:"flex",alignItems:"center",gap:6}}><span style={{fontSize:14}}>📋</span><span style={{fontSize:13,fontWeight:600,color:C.text}}>Today: {doneCount} of {_tasks.length} tasks done</span></div></div>{nextTask&&<div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginTop:6}}><span style={{fontSize:12,color:C.textMuted}}>Next: {nextTask.icon} {nextTask.label}</span>{nextTask.action&&<button onClick={nextTask.action} style={{padding:"4px 10px",borderRadius:6,background:C.tealBg,border:`1px solid ${C.teal}30`,color:C.teal,fontSize:11,fontWeight:600,cursor:"pointer",fontFamily:"inherit"}}>Start →</button>}</div>}</Card>;}catch{return null;}})()}
  ...
  <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>{(()=>{const daysPerWeek=getAssessment()?.preferences?.daysPerWeek||3;const consistencyPct=stats.sessionsThisWeek>0?Math.min(100,Math.round(stats.sessionsThisWeek/daysPerWeek*100)):0;return[{v:String(stats.totalSessions),l:"Days Done"},{v:`${stats.streak} 🔥`,l:"Streak"},{v:`${stats.sessionsThisWeek}/${daysPerWeek}`,l:"This Week",c:C.teal},{v:consistencyPct>0?`${consistencyPct}%`:"—",l:"Consistency",c:consistencyPct>=100?C.success:consistencyPct>=50?C.teal:C.textMuted}];})().map(s=>(<Card key={s.l} style={{textAlign:"center",padding:16}}><div style={{fontSize:s.v.length>3?28:36,fontWeight:800,color:s.c||C.text,fontFamily:"'Bebas Neue',sans-serif"}}>{s.v}</div><div style={{fontSize:11,color:C.textMuted,textTransform:"uppercase",letterSpacing:1.5}}>{s.l}</div></Card>))}</div>
...
function RecapScreen({onFinish,sessionData}){
  ...
  {sessionData?.exercisesCompleted?.length>0&&<Card style={{width:"100%"}}><div style={{fontSize:12,fontWeight:700,color:C.teal,letterSpacing:1.5,textTransform:"uppercase",marginBottom:8}}>YOUR EXERCISES</div>{sessionData.exercisesCompleted.map((ec,i)=>{const ex=exerciseDB.find(e=>e.id===ec.exercise_id);const wasSwapped=ec.swapped_from;return<div key={i} style={{display:"flex",alignItems:"center",gap:8,padding:"5px 0",borderBottom:`1px solid ${C.border}`}}><span style={{fontSize:12,width:16}}>{wasSwapped?"🔄":"✅"}</span><div style={{flex:1}}><div style={{fontSize:12,fontWeight:600,color:C.text}}>{ex?.name||ec.exercise_id}</div>{wasSwapped&&<div style={{fontSize:10,color:C.textDim}}>swapped from {wasSwapped}</div>}</div><div style={{fontSize:11,color:C.textMuted,textAlign:"right"}}>{ec.sets_done||1}×{ec.reps_done||"—"}{ec.load?` @ ${ec.load} lbs`:""}</div></div>})}</Card>}
