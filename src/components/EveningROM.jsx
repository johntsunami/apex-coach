    ...
    return (
      <div className="fade-in" style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 16, minHeight: 300, padding: "0 0 90px" }}>
        ...
        {(()=>{ const _nOv = getOverrideForExercise(next?.id); const _nImg = _nOv?.imageUrl || next?.imageUrl; return _nImg
          ? <img src={_nImg} alt={next.name} style={{ width: "60%", borderRadius: 12, border: `1px solid ${C.border}`, display: "block", objectFit: "cover", maxHeight: 160, background: "#080e1e" }} />
    ...
    <div className="fade-in" style={{ display: "flex", flexDirection: "column", gap: 12, padding: "0 0 90px" }}>
      ...
      <div style={{ background: C.bgCard, borderRadius: 16, padding: 16, border: `1px solid ${C.border}` }}>
        ...
        {(()=>{ const _ov = getOverrideForExercise(ex.id); const _img = _ov?.imageUrl || ex.imageUrl; return _img
          ? <><img src={_img} alt={ex.name} style={{ width: "100%", borderRadius: 12, border: `1px solid ${C.border}`, marginBottom: ex.videoUrl ? 6 : 12, display: "block", objectFit: "cover", maxHeight: 260, background: "#080e1e" }} onError={(e) => { e.target.style.display = "none"; e.target.nextSibling && (e.target.nextSibling.style.display = "block"); }} />
